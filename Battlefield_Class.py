...
class Battlefield():
    ...
    def _init_battlefield(self):
        ...
        self.coordinates, self.flag_angle = [
            [] * 2 for i in range(2)], [[] * 2 for i in range(2)]
        self.id, self.angle = [
            [] * 2 for i in range(2)], [[] * 2 for i in range(2)]
        self.flag = [0, 0]
    ...
    def operator_on_ships_button(self, _ship, event, N, K):
        ...
        if event.type == pygame.MOUSEMOTION:
            ...
            if (event.button == 3):
                ...
                x, y, angle_flag, d, angle = self.ship.examination_of_button(event, _ship, K)
                if (angle_flag != -1):
                    ...
                    if (flag):
                        ...
                        for i in range(size_ship[_ship] + 1):
                            if (angle_flag == 0):
                                self.battlefield[N][y][x +i] = size_ship[_ship] + 2
                            else:
                                self.battlefield[N][y +i][x] = size_ship[_ship] + 2

                        for i in range(len(Ships[_ship])):
                            if (angle_flag == 0):
                                self.battlefield[N][y + Ships[_ship][i][1]][x + Ships[_ship][i][0]] = 1
                            else:
                                self.battlefield[N][y + Ships[_ship][i][0]][x + Ships[_ship][i][1]] = 1
    ...
    def draw_battleground(self, N, screen_id, K):
        ...
        if (len(self.affected_cells_of_seas[N]) > 0):
            ...
            for i in range(len(self.affected_cells_of_seas[N])):
                ...
                center_ship = ((self.affected_cells_of_seas[N][i][1] - 1) * delta + y0 + delta//2 + 2 * delta, (
                    self.affected_cells_of_seas[N][i][0] - 1) * delta + x0 + delta//2 - 2 * delta)
        ...
        if (self.flag[N] == 1):
            for i in range(len(self.id[N])):
                ...
                x, y = self.coordinates[N][i][1] - \
                    1, self.coordinates[N][i][0] - 1
        ...
        if (len(self.affected_cells_of_ships[N]) > 0):
            for i in range(len(self.affected_cells_of_ships[N])):
                ...
                center_ship = ((self.affected_cells_of_ships[N][i][1] - 1) * delta + y0 + delta//2 + 2 * delta, (
                    self.affected_cells_of_ships[N][i][0] - 1) * delta + x0 + delta//2 - 2 * delta)
                x_rect = _x.get_rect(center=center_ship)
                screen.blit(_x, x_rect)
    ...
    def human_player_attack_exam(self,event,K):
        x0, y0 = K[0], K[1]
        if (event.type != pygame.MOUSEBUTTONDOWN):
            return False
        if (not (x0 <= event.pos[0] and event.pos[0] <= x0 + 10 * delta and y0 <= event.pos[1] and event.pos[1] <= y0 + 10 * delta)):
            return False
        return True

    def attack_on_ships(self, a, b ,N, type_player):
        if (self.battlefield[N][b][a] < 2):
